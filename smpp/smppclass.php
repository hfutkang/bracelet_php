...
// public members:
	...
	*/
	function Start($host, $port, $username, $password)
	{
/*
...
// private members (not documented):
	...
	function ExpectPDU($our_sequence_number)
	{
		do {
			...
			extract(unpack("Ncommand_id/Nsequence_number/Ncommand_status", $stream));
			...
			case CM_BIND_TRANSMITTER:
				...
				$this->debug("authen_source: $authen_source.\n");
				break;
			case CM_UNBIND:
	...
	function SendBindTransmitter($system_id, $smpppassword)
	{
		$system_id = $system_id;
		...
		$authenticator_str = $system_id."000000000".$smppassword.$time_stamp;
		$authenticator_source = md5($authenticator_str, true);
		$authen_len = strlen($authenticator_source);

		$pdu = pack("a{$system_id_len}a{$authen_len}a1N", $system_id, $authenticator_source, 48, intval($time_stamp));
		$this->debug("Bind Transmitter PDU: ");
		for ($i = 0; $i < strlen($pdu); $i++) {
			$this->debug(ord($pdu[$i]) . " ");
		}
		$this->debug($system_id." ".$time_stamp." ".$authenticator_source." ".$authen_len);
		$this->debug("\n");
